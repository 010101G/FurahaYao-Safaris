  ...
  return (
    <header id="top">
      ...
      >
        ...
        <nav
          className={`w-full h-[72px] transition-colors duration-300 box-border ${
            solid
              ? "bg-safari-cream/90 backdrop-blur-xl shadow-[0_1px_0_rgba(26,22,19,0.06)] text-safari-black"
              : "bg-transparent text-white"
          }`}
        >
        <div className="max-w-[1260px] mx-auto h-full px-4 md:px-6 flex items-center gap-4">
          {/* Logo */}
          <Link href="/" aria-label="FurahaYao Safaris home" className="flex items-center gap-3 flex-1">
            <span
              className={`grid place-items-center rounded-full p-1.5 transition-all duration-300 ${
                solid ? "bg-safari-black/5" : "bg-white/90 shadow-soft"
              }`}
            >
              <Image
            ...
            {links.map((l) =>
              l.children ? (
                ...
                >
                  <Link
                    href={l.href}
                    className={`relative px-3 py-2 text-sm font-semibold rounded-full transition-colors flex items-center gap-1 ${
                      isActive(l.href) ? (solid ? "text-safari-terracotta" : "text-safari-gold") : "hover:opacity-70"
                    }`}
                  >
                    {l.label}
          ...
          {/* Hamburger */}
          <button
            className="lg:hidden flex flex-col gap-1.5 ml-auto p-2 cursor-pointer"
            ...
            {[0, 1, 2].map((i) => (
              <span
                ...
                className={`block w-7 h-0.5 rounded-full transition-all duration-300 origin-center ${
                  solid ? "bg-safari-black" : "bg-white"
            ...
            <nav className="flex flex-col px-6 py-8 gap-1">
              ...
              >
                ...
                <div className="mt-5 flex items-center gap-3">
                  <span className="text-white/50 text-sm font-semibold">Language</span>
